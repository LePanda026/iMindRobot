...
# 创建名为thread_pool的线程池，线程数通过方法来获取
...
# 管理GPU内存，有CUDA用CUDA，没CUDA用Multi-Processing-Service
def torch_gc():
    import torch
    # 检查系统是否支持CUDA
    if torch.cuda.is_available():
        # with torch.cuda.device(DEVICE):
        # 如果支持，则清空GPU上的缓存，释放不在使用的内存
...
# 如果设备不在cuda，mps，cpu中，则自动检测并返回设备名称
def llm_device(device: str = LLM_DEVICE) -> Literal["cuda", "mps", "cpu"]:
    if device not in ["cuda", "mps", "cpu"]:
        device = detect_device()
    return device

# 函数的返回类型被注释为 Literal，这表示返回值只能是 "cuda"、"mps" 或 "cpu" 中的一个。
